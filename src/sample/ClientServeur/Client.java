        ...
        if (objectReaded instanceof Message) {
            ...
            if (message.getTypeRequest().equals("CHECK_IDENTIFIANT_AVAILABLE")) {
                ...
                if(response.get(0).equals("NOT_FIND") || response.get(0).equals("ALREADY_FRIENDS")) {
                    MainWindowController.addFriendsWindowController.setDataAddFriendsWindow(String.valueOf(response.get(0)), null);
                } else {
                    MainWindowController.addFriendsWindowController.setDataAddFriendsWindow(String.valueOf(response.get(0)), (User) response.get(1));
                    MainWindowController.users.add((User) response.get(1));

                }
            } else if(message.getTypeRequest().equals("GET_MESSAGE_SALON")) {
                if(message.getContenu() instanceof List) {
                    MainWindowController.principalWindowController.setConversationToMainWindow((List<Conversation>) message.getContenu());
                }
                ...
                if (message.getContenu() instanceof List){
                    ...
                    //MainWindowController.principalWindowController.afficheStats(reponse.get(0),reponse.get(1),reponse.get(2));
                    MainWindowController.principalWindowController.Threadstats(reponse.get(0),reponse.get(1),reponse.get(2));
                }
            } else if(message.getTypeRequest().equals("SO_CONNECTED")) {
                if(message.getContenu() instanceof List) {
                    List<String> allUsersConnected = (List<String>) message.getContenu();
                    MainWindowController.principalWindowController.friendsIsConnected(allUsersConnected);
                }
            }
        }
    }
